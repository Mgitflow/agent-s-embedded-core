...
# 数量词（与 functional_templates._QUANT_RE 同源：「」多实例）。
...
def parse_pin_reqs(text: str) -> dict[str, str | list[str]]:
    ...
    「识别到模板 → 拉默认角 → 识别到角不一样 → 替换」；本函数负责
    「识别角」这一步，产出 {参数键: 引脚 | 引脚列表}；配置环节据它替换默认角。

    多功能精准关联：按分隔符切子句 → 逐句「单功能 + 引脚」识别 →
    合并，让每个功能各带各的角。
    同功能多实例（「 PA5 和 PA3」）：数量词 + 单功能 + 多引脚 →
    ...
    if not text:
        return {}
    # 引脚代号（「把引脚画成代号」）：先抠「别名=引脚」定义，再把引用替换成物理引脚
