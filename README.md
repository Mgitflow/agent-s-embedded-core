...
| 🔒 **106 条校验** | 17 类外设工艺标准，拦截引脚冲突、时钟顺序、喂狗位置 |
...
一句话：**机制（代码）开源，材料（数据）闭源。**

骨架里的编排、106 校验、参数计算等**算法实现代码全开**；真正沉淀经验的具体数值（芯片引脚表、功能模板的 init 代码、手册电气数据）是护城河，这里只给**格式说明 + 空模板**，不夹带真实数据。

| 开什么 | 不开什么 |
|:---|:---|
| 确定性骨架（编排 / 106 校验 / 编译链路） | 芯片肖像数据（具体引脚 / AF 映射表，只给最小示例） |
| 算法机制代码（电气参数计算、外设参数反推等 `.py` 实现） | 功能模板数据（具体 init / loop 代码，只给 `example_led` 空模板） |
| 契约接口（数据怎么接入） | 手册 / 电气数据（时钟树、参数上下限） |
| 内容格式说明 + 空模板 | LLM 指挥层（可选外挂，可剥离） |
| 骨架自检脚本 | 测试脚本（无内容，测试无意义） |
| | 上板验证的完整数据 |
