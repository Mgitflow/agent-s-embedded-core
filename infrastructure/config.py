...
# 2026-08-06：config/settings.yaml 统一读取入口（server / assembly 共用，避免循环依赖与重复读取）
...
def get_chip_config(chip_name: str | None = None) -> dict[str, Any]:
    ...
    # 2026-08-12 修复（审查 P1）：此前无参 fallback "stm32f407"（skills/chips/ 下不存在）
...
def get_chip_skill_dir(chip_name: str | None = None) -> str:
    ...
    active = chip_name or ACTIVE_CHIP  # 2026-08-12 统一默认（此前 "stm32f407" 不存在）
...
# 工作区（编译产物归档根）
# 念安 8-21「编译产物放项目外，不被当垃圾」：默认写到项目外同级目录
...
# ── LLM 双通道（2026-08-06）：本地 LM Studio 直连 ↔ 云端 OpenAI 兼容 API ──
...
# 2026-08-06 修根：REFLUX_DIR 支持 AGENT_S_REFLUX_DIR 环境变量覆盖——
...
# ═══════════════════════════════════════════
# 核心数据根（2026-08-19 拔插式改造：open-core 护城河）
...
# （2026-08-19 念安定：手册资料不塞芯片肖像目录，单独开 knowledge/manuals/<chip>/，
...
# 回流（沉淀日志）——2026-08-06 修根：全部从 REFLUX_DIR（支持 AGENT_S_REFLUX_DIR
# 环境变量覆盖）派生，测试隔离 reflux 数据时一处设置全生效。
KNOWLEDGE_RAW_PATH = str(PROJECT_ROOT / "knowledge" / "crawled")  # 2026-08-06 补：integrator 原始知识目录（从 E 抄 integrator 时漏配）
...
# 2026-08-11 念安拍板：reference（28 万行 STM32 HAL 库）移出主仓库，
# 作为本地附属库位于 E:/Code/reference-stm32f4/（环境变量 S_REFERENCE_DIR 可覆盖）
# 2026-08-14 芯片数据扩充（F103/G431）：HAL 库按芯片系列分区
...
def reference_dir_for_family(family: str) -> Path:
    ...
    2026-08-14 智能巡查点：统一委托 chip_gateway 索引（唯一事实源）。
...
# ═══════════════════════════════════════════
# 外部组件路径（2026-08-19 集中化：open-core 拔插式）
